...
<Tabs items={['PHP/Laravel', 'JavaScript', 'Flutter', 'cURL']}>
  ...
  <Tabs.Tab>
    ```dart
    // Flutter - Fetch zones and display on map
    Future<List<Zone>> fetchServiceZones() async {
      final response = await http.get(
        Uri.parse('https://currydash.au/api/v1/zone/list'),
      );

      if (response.statusCode == 200) {
        final List<dynamic> jsonZones = json.decode(response.body);
        return jsonZones.map((json) => Zone.fromJson(json)).toList();
      } else {
    ...
    // Render zones on Google Maps
    Set<Polygon> _buildZonePolygons(List<Zone> zones) {
      return zones.map((zone) {
        return Polygon(
          polygonId: PolygonId('zone_${zone.id}'),
          points: zone.coordinates.map((c) =>
            LatLng(c.lat, c.lng)
          ).toList(),
          strokeWidth: 2,
...
<Tabs items={['PHP/Laravel', 'JavaScript', 'Flutter', 'cURL']}>
  ...
  <Tabs.Tab>
    ```dart
    // Flutter - Check zone availability
    Future<ZoneCheckResult?> checkZoneAvailability(
      double latitude,
      double longitude
    ) async {
      final url = 'https://currydash.au/api/v1/zone/check'
      ...
      try {
        ...
        if (response.statusCode == 200) {
          return ZoneCheckResult.fromJson(json.decode(response.body));
        } else if (response.statusCode == 404) {
          // Not serviceable
          return null;
        } else {
          throw Exception('Failed to check zone');
        }
      } catch (e) {
    ...
    ZoneCheckResult? zone = await checkZoneAvailability(
      position.latitude,
      position.longitude
    );

    if (zone != null && zone.serviceable) {
      // Load restaurants for this zone
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (context) => RestaurantListScreen(zoneId: zone.zoneId),
        ),
      );
    } else {
      // Show not serviceable dialog
      showDialog(
        context: context,
        builder: (context) => AlertDialog(
          title: Text('Service Unavailable'),
          content: Text('We don\'t deliver to this area yet. Check back soon!'),
        ),
      );
    }
    ```
